...
public class UserController extends BaseController {
    ...
    public Response regist(User user) {
        try {
            ...
            // 加入預設收藏夾
			/*Favorites favorites = favoritesService.saveFavorites(user.getId(), "未讀清單");
			// 加入預設屬性設定
   ...
   /* @RequestMapping(value="/activeuserEmail",method=RequestMethod.GET)
    ...
    public String activeuserEmail(Model model, String email, String sid) {
        try {
            ...
            if (outDate.getTime() <= System.currentTimeMillis()) { //表示已經由期
                model.addAttribute("states", "簽名過期");
                return "user/activeuserEmail";
//                System.out.print("過期");
            }
            String key = user.getEmail() + "$" + outDate.getTime() / 1000 * 1000 + "$" + user.getValidataCode();//數字簽名
            ...
            if (!digitalSignature.equals(sid)) {
                model.addAttribute("states", "簽名不對");
                return "user/activeuserEmail";
...
//            return result(ExceptionMsg.FAILED);
        ...
        model.addAttribute("states", "啟動成功");
        return "user/activeuserEmail";
