...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    /**
     *  Disable spring security (for API testing)
     *      https://stackoverflow.com/questions/47273691/spring-boot-2-0-disable-default-security
     */
    @Override
    protected void configure(HttpSecurity http) throws Exception{
        http.authorizeRequests().antMatchers("/").permitAll() // disable spring security
                .and().cors().and().csrf().disable(); // disable CSRF (can accept Post request)
    }
