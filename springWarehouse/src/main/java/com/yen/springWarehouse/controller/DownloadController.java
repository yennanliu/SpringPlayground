...
public class DownloadController {
  ...
  @Autowired DownloadStatusService downloadStatusService;

  @Autowired DownloadStatusMapper downloadStatusMapper;
  ...
  public String list(
    ...
    IPage<DownloadStatus> iPage =
        downloadStatusService.page(
            page, new LambdaQueryWrapper<DownloadStatus>().orderByAsc(DownloadStatus::getId));
  ...
  public ResponseEntity<Resource> downloadFile(String url) throws IOException {
    ...
    //        log.info(resource.getPath() + String.valueOf(resource.exists()) +
    // String.valueOf(resource.getURL()));

    HttpHeaders headers = new HttpHeaders();
    // headers.add(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=%s".format("dummy.json"));
    headers.add(HttpHeaders.CONTENT_DISPOSITION, String.format(downloadUrl));
