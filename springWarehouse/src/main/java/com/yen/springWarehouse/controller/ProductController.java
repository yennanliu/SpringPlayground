...
public class ProductController {
  ...
  public String list(
      ...
      ProductQueryHelper helper) {

    log.info(
        ">>> (ProductController list) pageNoStr = {}, map = {}, helper = {}",
        pageNoStr,
        map,
        helper);
    int pageNo;
  ...
  public String update(Product product, Map<String, Object> map) throws Exception {
    ...
    try {
      // TODO : fix if modify as invalid/not existed typeId (### Error updating database.  Cause:
      // java.sql.SQLIntegrityConstraintViolationException: Cannot add or update a child row: a
      // foreign key constraint fails (`warehouse_system`.`product`, CONSTRAINT `FK_PRODUCT_TYPE`
